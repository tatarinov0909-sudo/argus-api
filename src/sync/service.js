...
async function upsertProducts(client, warehouseId, records) {
  ...
  for (const rec of records) {
    ...
    // Новый товар без продавца всё равно заводим — без владельца. Раньше он
    // пропускался, а назначить продавца потом было нечему: документ 1С с
    // контрагентом ищет уже существующую карточку. Так у нового продавца
    // (Авезов, 24.09.2026) не доехали две позиции на 10 838 шт. Карточка без
    // владельца продавцам не видна; владельца ей назначит документ с
    // контрагентом или загрузка остатков продавца.
